...
resource "aws_db_instance" "read_replica" {
  ...
  identifier = "${var.identifier}-readreplica"
  engine            = var.engine
  engine_version    = var.engine_version
  instance_class    = var.instance_class
  allocated_storage = var.allocated_storage
  ...
  apply_immediately           = "true"
  maintenance_window          = var.maintenance_window           # check if this is neccesary - final_snapshot_identifier
  skip_final_snapshot         = var.skip_final_snapshot
  final_snapshot_identifier   = "${var.final_snapshot_identifier}-readreplica"

  backup_retention_period = 0          # check if this is neccesary - final_snapshot_identifier
}
...
resource "aws_db_instance" "this_mssql" {
  ...
  #   default     = true
  # }
  # local.is_mssql = false
  # count = true && false ? 1 : 0
  # count = true && false ? 1 : 0
  # count = false ? 1 : 0      #   true ... therefore count =1 
